...
from utils.keras import TensorSpec, replace_kwargs
from .generation_utils import infer
from .current_blocks import Conv2DBN, set_cudnn_lstm
from .layers import get_activation, CustomRNNDropoutCell, CustomEmbedding
...
CRNNStep  = collections.namedtuple(
    'CRNNStep', ['output', 'attention_weights', 'state']
)
CRNNOutput = collections.namedtuple(
    'CRNNOutput', ['tokens', 'lengths', 'scores', 'logits', 'attention_weights']
)

_default_vgg_layers =  [
    32, 'M', 64, 'M', 128, 128, ('M', (2, 1)), (256, 3, False), 'B', (256, 3, False), 'B',
    ('M', (2, 1)), (256, 2)
]    

separator_char = ['\xa2', '\xa3'] + ['\xa4', '\xa5']
...
def ResNetBackbone(input_shape     = (None, None, 1),
                   input_tensor    = None,
    ...
    for i, (n_blocks, channels, k, s) in enumerate(zip(blocks, channels, kernel_size, strides)):
        ...
        for j in range(2 if i in (0, 4) else 1):
            x = Conv2DBN(
                x,
                filters = channels if not isinstance(channels, list) else channels[j],
                kernel_size = k,
                use_bias    = False,
                strides     = s if i in (0, 4) and j == 0 else 1,
                ...
                name    = name_format.format(name = name, i = i, j = j + 1),
...
def CRNNCTC(input_shape    = None,
    ...
    if pretrained or pretrained_lang:
        pretrained, infos = _load_easyocr_crnn(pretrained, pretrained_lang)
...
class CRNNWithAttn(keras.Model):
    def __init__(self,
        ...
        kwargs.setdefault('pretrained_lang', None)
        kwargs.setdefault('backbone', 'resnet')
    ...
    def call(self, inputs, training = False):
        ...
        encoded_mask = getattr(encoded, '_keras_mask', None)
    ...
    def step(self,
        ...
        return CRNNStep(
            output = logit,
            state  = CRNNState(attention_state = state, main_attention = main_attention),
            attention_weights = attn
        )
    
    @replace_kwargs(infer, ignore = {'encoder_output', 'is_transformer', 'step_fn'})
    def infer(self,
    ...
    def from_config(cls, config, custom_objects = None):
        return cls(** {** config, 'pretrained' : None, 'pretrained_lang' : None})
...
def _get_easyocr_crnn_infos(model = None, lang = 'english', gen = 2):
    if model is None:
        for model, infos in EASYOCR_MODELS['gen{}'.format(gen)].items():
            if lang in _lang_groups.get(model[:-3], []) or lang == model[:-3]:
                return _get_easyocr_crnn_infos(model = model, gen = gen)
        
        raise ValueError('No model found for language {} in gen {}'.format(lang, gen))

    return model, EASYOCR_MODELS['gen{}'.format(gen)][model]

def _load_easyocr_crnn(model = None, lang = 'english', gen = 2):
    import torch
    import zipfile
    ...
    model, infos = _get_easyocr_crnn_infos(model = model, lang = lang, gen = gen)
